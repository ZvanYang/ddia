...
为了扩展到多个CPU内核和多个节点，您可以潜在地对数据进行分区，（请参阅[第六章](ch6.md)），在VoltDB中支持这样做。如果你可以找到一种对数据集进行分区的方法，以便每个事务只需要在单个分区中读写数据，那么每个分区就可以拥有自己独立运行的事务处理线程。在这种情况下，你可以为每个CPU内核分配自己的分区，这允许你的事务吞吐量随CPU内核数量线性扩展【47】。
